...
#include <stdio.h>
int posting_insert(struct posting *p,unsigned int d,double w);
struct posting* posting_alloc(char *c,unsigned int d,double w);
...
void invertedlist_print(char *c,unsigned int len)
{
	...
	l = vbyte_decompress(c+pos,c+len,&lastdoc);
	pos += l;
	printf("<%d,%f>",lastdoc,*(double*)(c+pos));
	pos += sizeof(double);
...
int main()
{
	...
	p = posting_alloc("test",4,0.1);
	if(p == NULL)
		return -1;
	posting_insert(p,5,0.2);
	ret = merge_invertedlist(p,NULL,0,&retlen);

	invertedlist_print(ret,retlen);

	p2 = posting_alloc("test",7,0.1);
