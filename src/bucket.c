...
static int _bucket_find(struct bucket *bkt,const char *term,unsigned short *pos)
{
...
char* bucket_find(struct bucket *bkt,const char *term,unsigned int *invertedlen)
{
	unsigned short pos;
...
int bucket_remove(struct bucket *bkt,char *term)
{
	unsigned short pos;
	unsigned int len;
	char *posptr;
...
int bucket_insert(struct bucket *bkt,char *term,void *invertedlist,unsigned short invertedlen)
{
	unsigned short pos;
	int termlen;
	int big;
...
int bucket_splite(struct bucket *old,struct bucket *new)
{
	...
	while(i < old->head->num)
	{
		...
		if(first == 0)
		{
			mask = hash(term)%(1<<old->head->local_depth);
			first = 1;
		}
		else if(old->head->local_depth != 0)
		{

			assert(mask == hash(term)%(1<<old->head->local_depth));
		}
#endif

		if((hash(term) & (1<<old->head->local_depth)) != 0) /* the significant bit is set,
...
int main()
{
	...
	if(bucket_insert(&bkt,term,invertedlist,strlen(invertedlist)) != 0)
	{
		printf("error bucket_insert\n");
		goto free_bucket;
	}
	if(bucket_insert(&bkt,term2,invertedlist,strlen(invertedlist)) != 0)
	{
		printf("error bucket_insert\n");
		goto free_bucket;
	}
	...
	if(bucket_remove(&bkt,term) != 0)
	{
		printf("error remove\n");
		goto free_bucket;
	}
	if(bucket_insert(&bkt,"wod",invertedlist,strlen(invertedlist)) != 0)
	{
		printf("error bucket_insert\n");
		goto free_bucket;
	}
	if(bucket_insert(&bkt,"bbcnews",invertedlist,strlen(invertedlist)) != 0)
	{
		printf("error bucket_insert\n");
		goto free_bucket;
	}
	if(bucket_insert(&bkt,"bews",invertedlist,strlen(invertedlist)) != 0)
	{
		printf("error bucket_insert\n");
		goto free_bucket;
	}
	if(bucket_insert(&bkt,"omga",invertedlist,strlen(invertedlist)) != 0)
	{
		printf("error bucket_insert\n");
		goto free_bucket;
	}
	if(bucket_insert(&bkt,"pnwh",invertedlist,strlen(invertedlist)) != 0)
	{
		printf("error bucket_insert\n");
		goto free_bucket;
	}
	if(bucket_insert(&bkt,"cnews",invertedlist,strlen(invertedlist)) != 0)
	{
		printf("error bucket_insert\n");
		goto free_bucket;
	}
