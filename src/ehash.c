...
char *ehash_find(struct ehash *h,const char *c,unsigned int *len)
{
	unsigned int entry;
	struct page *page;
...
int ehash_insert(struct ehash* h,char *c,char *invertedlist,unsigned short invertedlen)
{
	unsigned int entry;
	unsigned int hashval;
	struct page *page;
	...
	{
		if(h->bucket->head->local_depth < h->global_depth) /* spilte bucket */
		{
			size = PAGE_SIZE;
			if(freemap_malloc(h->freemap,&size,&fd,&offset) != 0)
				goto exit;
